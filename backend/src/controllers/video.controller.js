...
// ─────────────────────────────────────────────
// CHUNK UPLOAD
// Frontend sends chunks one by one
// ─────────────────────────────────────────────
const uploadChunk = async (req, res) => {
  try {
    ...
    if (!chunk) {
      return res.status(400).json({ success: false, message: 'No chunk received.' });
    }
...
const mergeChunks = async (req, res) => {
  try {
    ...
    for (let i = 0; i < parseInt(totalChunks); i++) {
      ...
      if (!fs.existsSync(chunkPath)) {
        return res.status(400).json({ 
          success: false, 
          message: `Missing chunk ${i}. Upload incomplete.` 
        });
      }
    ...
    const [result] = await db.query(
      `INSERT INTO videos 
        (title, description, type, genre, language, release_year, file_path, thumbnail_path, file_size, created_by) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        title,
        description || null,
...
// ─────────────────────────────────────────────
// STREAM VIDEO
// Supports range requests (seek/resume)
// ─────────────────────────────────────────────
  ...
  try {
    ...
    if (!users[0].is_subscribed) {
      return res.status(403).json({
        success: false,
        message: 'Subscription required to watch content.'
      });
    }
...
const cancelUpload = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Upload cancelled and temp files cleaned.' });
