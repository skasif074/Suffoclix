      ...
      <div style={{ padding: '0 0 60px' }}>
        ...
        {!searchQuery && continueWatching.length > 0 && (
          <PlaylistRow
            ...
            videos={continueWatching.map(v => ({ ...v, id: v.video_id, progress_percent: v.progress_percent }))}
        ...
        {!searchQuery && playlists.length === 0 ? (
          <div className="empty-state" style={{ marginTop: '60px' }}>
            ...
            {isAdmin && (
              <Link to="/admin" className="btn btn-primary" style={{ marginTop: '20px' }}>+ Create Playlist</Link>
            )}
          </div>
        ) : (
  ...
  return (
    <div style={{ marginBottom: '40px', position: 'relative' }}>
      <div style={{
        ...
        padding: '0 48px', marginBottom: '14px',
      }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', flex: 1, minWidth: 0 }}>
          <div style={{
            width: '4px', minHeight: '22px',
            background: 'var(--gradient)', borderRadius: '2px', marginTop: '3px', flexShrink: 0,
          }} />
        ...
        {playlistId && (
          <Link to={`/playlist/${playlistId}`} style={{
            ...
            gap: '4px', opacity: 0.8, transition: 'opacity 0.2s', flexShrink: 0, marginLeft: '16px',
          }}
...
const VideoCard = ({ video, index, isSubscribed, isAdmin }) => {
  ...
  return (
    <Link to={canWatch ? `/video/${video.id}` : '/subscribe'} style={{ textDecoration: 'none', flexShrink: 0 }}>
      <div
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        style={{
          width: '180px', transition: 'transform 0.2s ease',
      ...
      >
        <div style={{
          ...
          {index !== undefined && (
            <div style={{
              ...
              padding: '2px 6px', fontSize: '11px', fontWeight: '700', color: 'var(--text-secondary)',
            }}>
              {index + 1}
            </div>
          )}
          <div style={{
            ...
            borderRadius: '4px', padding: '2px 6px', fontSize: '9px', fontWeight: '700',
            color: video.type === 'movie' ? 'white' : '#000', textTransform: 'uppercase',
          }}>
        ...
        <div style={{ padding: '0 2px' }}>
          ...
          {video.genre && (
            <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{video.genre}</div>
          )}
        </div>
      </div>
    </Link>
  ...
  return (
    <Link to={`/video/${video.id || video.video_id}`} style={{ textDecoration: 'none', flexShrink: 0 }}>
      <div
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        style={{ width: '220px', transition: 'transform 0.2s', transform: hovered ? 'scale(1.04)' : 'scale(1)' }}
      >
        <div style={{
          width: '220px', height: '124px', borderRadius: '8px',
          background: video.thumbnail_path
            ? `url(${apiBase}/${video.thumbnail_path}) center/cover`
            : 'linear-gradient(135deg, #1a1a2e, #16213e)',
          position: 'relative', overflow: 'hidden', marginBottom: '6px',
          border: hovered ? '2px solid var(--accent)' : '2px solid transparent',
          transition: 'border 0.2s',
        }}>
          {!video.thumbnail_path && (
          ...
          <div style={{
            ...
            <div style={{ height: '100%', width: `${video.progress_percent || 0}%`, background: 'var(--accent)' }} />
          </div>
        </div>
        <div style={{
          fontSize: '13px', fontWeight: '600',
